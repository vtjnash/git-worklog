...
function import_url!(st::BState, raw::AbstractString, at::DateTime)
    ...
    # done mark - and an undo that took back only the field left the row in the
    # unread lane for good: no poll covers the repo, which is why it was
    # imported, so nothing would ever have cleared it. The mark is both halves:
    # `mark_unread` drops the head with the stamp, and an undo that put back
    # the stamp alone brought the thread's rule back with `p` still saying
    # "nothing to compare against".
    hadrow = Events.in_inbox(u)
    prevread = mark_at(u, "done")            # raw: an undo puts back what was said
    prevhead = mark_at(u, "done_head")
    ...
    push_undo!(st, Undo(string("import ", it.ref), u, () -> begin
        ...
        # With `fold`, as `e`'s undo: an absent stamp and a head is what goes
        # back, not the head dropped for want of the stamp.
        set_done_mark(u, prevread, prevhead; fold = true)
    ...
    end; keys = ["imported", "done", "done_head", "touched"],
         redo = () -> begin
